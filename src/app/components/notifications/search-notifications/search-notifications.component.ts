...
@Component({
  ...
  styleUrls: ['./search-notifications.component.less']
})
export class SearchNotificationsComponent implements OnInit {

  public notifications: Notification[] = [];

  public showNotifications: Notification[] = [];
  ...
  private async loadData(): Promise<void> {
    try {
      const request: any = await this.http.post(environment.api + '/api/admin/notifications/get-notifications', {
      ...
      if(request.status === 'ok') {
        this.notifications = request.responseData.notifications;
        this.refreshList();
      }
      else {
      ...
      this.errorHandlerService.throwError({
        displayMessage: "Could not load the notifications.",
  ...
  public refreshList(): void {
    ...
    this.showNotifications = this.notifications.slice(skip, skip + this.resultsPerPage);

    this.setPageNumbers();
  }

  public setPageNumbers(): void {
    const totalPageCount = Math.ceil(this.notifications.length / this.resultsPerPage);
  ...
  public async searchNotifications(): Promise<void> {
    try {
      ...
      this.errorHandlerService.throwError({
        displayMessage: 'Could not search notifications.',
        unsanitizedMessage: error
      });
