...
@Component({
  ...
  templateUrl: './search-influencers.component.html',
  styleUrls: ['./search-influencers.component.less']
})
export class SearchInfluencersComponent implements OnInit {

  public influencers: Influencer[] = [];

  public showInfluencers: Influencer[] = [];
  public totalPageCount: number = 0;
  public totalInfluencerCount: number = 0;
  ...
  public influencerDeleted: boolean = false;
  ...
  constructor(
    ...
    console.log("create influencer component loaded")
    if(this.router.getCurrentNavigation().extras.state) {

      if(this.router.getCurrentNavigation().extras.state.influencerDeleted) {
        this.influencerDeleted = true;

        setTimeout(() => {
          this.influencerDeleted = false;
        }, 3000);
      }
    }
  ...
  public refreshList(): void {
    ...
    this.showInfluencers = this.influencers.slice(skip, skip + this.resultsPerPage);

    this.setPageNumbers();
  }

  public setPageNumbers(): void {
    const totalPageCount = Math.ceil(this.influencers.length / this.resultsPerPage);
  ...
  public async searchInfluencers(): Promise<void> {
    try {
      ...
      this.errorHandlerService.throwError({
        displayMessage: 'Could not search influencers.',
        unsanitizedMessage: error
      });
