...
@Component({
  selector: 'app-search-support-tickets',
  templateUrl: './search-support-tickets.component.html',
  styleUrls: ['./search-support-tickets.component.less']
})
export class SearchSupportTicketsComponent implements OnInit {

  public tickets: Ticket[] = [];

  public showTickets: Ticket[] = [];
  ...
  private async loadData(): Promise<void> {
    try {
      const request: any = await this.http.post(environment.api + '/api/admin/support-tickets/get-support-tickets', {
      ...
      if(request.status === 'ok') {
        this.tickets = request.responseData.supportTickets;
        this.refreshList();
      }
      else {
  ...
  public refreshList(): void {
    ...
    this.showTickets = this.tickets.slice(skip, skip + this.resultsPerPage);

    this.setPageNumbers();
  }

  public setPageNumbers(): void {
    const totalPageCount = Math.ceil(this.tickets.length / this.resultsPerPage);
  ...
  public async resolveTicket(): Promise<void> {
    try {
      ...
      if(request.status === 'ok') {
        ...
        const ticketIndex = this.tickets.findIndex((x) => x.id === this.pendingResolveTicketId);
